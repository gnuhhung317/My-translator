...
class PraticeWindow:
    def __init__(self):
        ...
        self.words = []         #list of words choosen for practicing
        self.answer = []        #
        self.true_answer = []
        self.indice = 0
        ...
        self.window.config(bg = "DodgerBlue1")
    ...
    def practice(self):
        ...
        label = Label(self.window,text="PRACTICE",font=("Arial",24),background="LightBlue1")
        label.grid(row=0,column=0,sticky=EW,columnspan=3)
        self.word_label = Label(self.window,text=f"{self.words[self.indice][0]}",font=("Arial",18),background="LightSteelBlue2")
        ...
        self.backward_button = Button(self.window,text="<<",command=self.backward,background="LightSteelBlue3")
        self.backward_button.grid(row=3,column=0,padx=20,pady=20)
        self.forward_button = Button(self.window,text=">>",command=self.forward,background="LightSteelBlue3")
        self.forward_button.grid(row=3,column=2,padx=20,pady=20)
        self.submit_button = Button(self.window,text="Submit",font=("Arial",18),command=self.submit,background="LightSteelBlue3")
        self.submit_button.grid(row=3,column=1,padx=20,pady=20)

        self.index_label = Label(self.window,relief=SUNKEN,text=f"{self.indice+1} of {self.words_quantity}",background="LightSteelBlue3")
    ...
    def reload(self):
        ...
        self.word_label = Label(self.window,text=f"{self.words[self.indice][0]}",font=("Arial",18),background="LightSteelBlue3")
        self.word_label.grid(row=1,column=1,padx=10,pady=10)
    ...
    def ask_quantity(self):
        ...
        self.quantity_label = Label(
            self.quantity_frame, text="Enter the words's number: ", font=("Arial", 14),background="LightSteelBlue1")
        self.quantity_label.grid(row=0, column=0,rowspan=3)
        self.quantity_entry = Entry(self.quantity_frame, font=("Arial", 14))
        self.quantity_entry.grid(row=0, column=1,rowspan=3)
        self.quantity_button = Button(
            self.window, text="Confirm", command=self.confirm)
        self.quantity_button.grid(row=3,column=0,padx=10,pady=10)
    ...
    def make_key_table(self):
        ...
        for row,word in enumerate(self.words):
            Label(self.window,text=f"{word[0]}").grid(row=row+2,column=0,padx=10,pady=10)
            Label(self.window,text=f"{word[1]}").grid(row=row+2,column=1,padx=10,pady=10)
        for row,word in enumerate(self.answer):
            Label(self.window,text=f"{word}").grid(row=row+2,column=2,padx=10,pady=10)
